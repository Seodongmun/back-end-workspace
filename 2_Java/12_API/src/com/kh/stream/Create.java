...
/*
   ...
   - java.util.stream 패키지에 존재하고 BaseStream 인터페이스를 
     부모로 갖고 자식 인터페이스들이 상속 관계를 이루고 있다.
...
//		첫번째 매개값(1) ~ 두번째 매개 값 이전까지의 값(10)을 요소로 가지는 스트림 객체 생성
		IntStream stream = IntStream.range(1, 10); // 1 ~ 9
		int sum = stream.sum();
		System.out.println("range의 sum : " + sum); // 45
		
//		첫번째 매개값(1) ~ 두번째 매개 값(10)까지의 값을 요소로 가지는 스트림 객체 생성
