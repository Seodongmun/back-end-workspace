...
//		list.add(3);
	...
	public void method2() {
		...
		// 위의 잘라낸 sub를 addAll에 추가해서 출력
		list.addAll(sub);
		System.out.println(list);

		// 9. isEmpty : 컬렉션이 비어있는지
		System.out.println("리스트가 비어있는가? " + list.isEmpty()); // fasle
		...
		System.out.println("평균연령은 " + avg / list.size() + "살 입니다");
		...
		System.out.println("나이 순서대로 출력" + list);
...
//							가지고 비교값들을 반환하여 정렬
		...
		// 12. 내림차순 정렬 : sort 메서드를 통해서
//							오름차순 정렬 후 reverse 메서드를 호출하여 정렬
		Collections.reverse(list);
		// reverse만할경우 알파벳 순서가 아닌 값을 대입한 기준으로 내림차순
		System.out.println(list);
