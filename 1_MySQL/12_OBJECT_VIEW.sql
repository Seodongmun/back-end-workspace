...
-- '한국'에서 근무하는 사번들의 사번, 이름, 부서명, 급여, 근무국가명 조회 
...
-- '러시아'에서 근무하는 사번들의 사번, 이름, 부서명, 급여, 근무국가명 조회 
SELECT emp_id, emp_name, dept_title, salary, national_name
FROM employee
	JOIN department ON (dept_id = dept_code)
	JOIN location ON (local_code = location_id)
...
-- '일본'에서 근무하는 사번들의 사번, 이름, 부서명, 급여, 근무국가명 조회 
SELECT emp_id, emp_name, dept_title, salary, national_name
FROM employee
	JOIN department ON (dept_id = dept_code)
	JOIN location ON (local_code = location_id)
...
AS SELECT dept_code 부서코드, format(sum(salary),0)부서별합계 , format(avg(salary),0)부서별평균
	FROM employee
	GROUP BY dept_code;
...
VALUES(500, '송준호', '629434-1234567','총무부');
