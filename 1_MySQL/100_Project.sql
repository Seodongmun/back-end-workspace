...
CREATE TABLE user ( -- 유저
    ...
    user_manner DECIMAL(2,2) DEFAULT (36.5), -- 유저온도 0.00 ~ 99.99까지가능
...
CREATE TABLE membership ( -- 클럽
    ...
    membership_grade DECIMAL(2,2), -- 클럽 별점
    membership_max INT, -- 클럽최대인원
    mem_loc_code INT, -- 클럽 지역 리스트
    mem_type_code INT -- 클럽 유형 리스트
);
...
ALTER TABLE membership ADD  FOREIGN KEY (mem_loc_code) REFERENCES membership_location(mem_loc_code);
ALTER TABLE membership ADD  FOREIGN KEY (mem_type_code) REFERENCES membership_type(mem_type_code);
...
ALTER TABLE comment ADD  FOREIGN KEY (comment_com_code) REFERENCES comment(comment_code);
