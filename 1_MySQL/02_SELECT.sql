/*
    ...
    - **Create 추가 Read 보기 Update 수정 Delete 삭제
    ...
    5. TCL(Transaction Control Languate) : 트랜잭션을 제어하는 언어
...
/*
	...
	- 별칭을 부여할 때 띄어쓰기 혹은 특수문자가 포함될 경우 반드시 큰 따옴표("")로 감싸준다.
...
SELECT emp_name, emp_id, hire_date, ent_yn
FROM employee
WHERE ent_yn = 'N';
...
-- 전화번호의 3번째 자리가 1인 사원들의 사번(emp_id), 사원명, 전화번호, 이메일 조회
...
-- 사번(emp_id), 이름(emp_name), 급여(salary), 보너스(bouns) 조회 
...
FROM employee
-- ORDER BY bonus; -- 보너스 기준으로 오름차순 정렬 (NULL이 맨앞)
ORDER BY 2 DESC; -- 보너스 기준으로 내림차순 정렬 (NULL이 맨뒤)
...
--    급여가 200만원 이상인 사람들의 모든 컬럼 조회
SELECT *
FROM employee
WHERE job_code IN ('J7','J2') AND salary >= '2000000';


-- 2. 사수가 없고 부서배치도 받지 않은 사원들의 사원명
--    사수사번(manager_id), 부서코드 조회
...
FROM employee
WHERE 2000000 <= salary AND salary <= 5000000 AND hire_date >= '2001-01-01' AND bonus IS NULL;
...
FROM  employee
WHERE (salary * 12) AND bonus IS NOT NULL AND emp_name LIKE '%하%';
