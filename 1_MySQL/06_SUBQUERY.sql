...
-- 1) 전 직원의 평균 급여 / 보다 더 많은 급여 / 를 받는 사원들의 사번, 사원명, 직급코드, 급여 조회
SELECT * FROM employee;
SELECT * FROM job;
SELECT emp_id, emp_name, job_code, format((salary),0) 평균급여
FROM employee
...
SELECT emp_name, emp_id, phone, job_name, dept_title, hire_date
FROM employee
JOIN department ON (dept_code=dept_id)
JOIN job USING (job_code)
...
SELECT max(salary)
FROM employee
GROUP BY dept_code; -- 28900000, 3660000, 2490000, 3760000, 3900000, 2550000, 8000000
...
-- 직원 에 대한 / 사번, 이름, 부서코드, 구분(사수/사원) 조회 
SELECT * FROM employee;
SELECT * FROM department;
SELECT * FROM job;
...
-- 1) 중복되지않고 NULL이 아닌 매니저 값.
SELECT DISTINCT manager_id
FROM employee
WHERE manager_id IS NOT NULL;

-- 2) 직원 에 대한 / 사번, 이름, 부서코드, 구분(사수/사원) 조회 
...
/*
    ...
    --> OR
*/
-- 대리 직급임에도, 과장 직급들의 최소 급여 / 보다 많이 받는 직원의 사번, 이름, 직급, 급여 조회
SELECT * FROM employee;
SELECT * FROM department;
SELECT * FROM job;

-- 1) 과장보다 더 많이 받는 대리 직급
SELECT job_name,salary, emp_id, emp_name
FROM employee
JOIN job USING (job_code)
...
*/
-- 과장 직급임에도 차장 직급의 최대 급여 / 보다 더 많이 받는 사원들의 사번, 이름, 직급, 급여 조회
SELECT * FROM employee;
SELECT * FROM department;
SELECT * FROM job;

-- 차장 직급의 최대 급여 --2800000
...
FROM employee
	JOIN job USING (job_code)
WHERE job_name = '과장' and salary > ALL (SELECT salary -- 서브쿼리의 결과값을 일치시켜야 정확한 값이 나온다!
										FROM employee
										JOIN job USING (job_code)
										WHERE job_name = '차장');
...
*/
-- 하이유 사원  과 같은 부서 코드 / , 같은 직급 코드에 해당하는 사원들의 사원명, 부서코드, 직급코드 조회 
SELECT * FROM employee;
SELECT * FROM department;
SELECT * FROM job;
...
-- 박나라 사원과 직급 코드 (J7)가 일치하면서
SELECT job_code , manager_id
FROM employee
WHERE emp_name = '박나라';

-- 같은 사수를 가지고 있는 사원의 사번, 이름, 직급코드 사수사번 조회
SELECT emp_id, emp_name, job_code, manager_id'사수'
FROM employee
